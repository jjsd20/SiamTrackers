...
class SiamRPNNet(nn.Module):
    ...
    def forward(self, template, detection):
        ...
        template_feature = self.featureExtract(template)#[16,256,6,6]
        
        detection_feature = self.featureExtract(detection)#[16,256,24,24]

        kernel_score = self.conv_cls1(template_feature).view(N, 2 * self.anchor_num, 256, 4, 4)    #16,2*5,256,4,4
        
        kernel_regression = self.conv_r1(template_feature).view(N, 4 * self.anchor_num, 256, 4, 4) #16,4*5,256,4,4
       
        conv_score = self.conv_cls2(detection_feature) #16,256,22,22#对齐操作
        conv_regression = self.conv_r2(detection_feature)#16,256,22,22

        conv_scores = conv_score.reshape(1, -1, self.score_displacement + 4, self.score_displacement + 4)#1，16*256,22,22
        # 1，8192,22,22
        score_filters = kernel_score.reshape(-1, 256, 4, 4) #16x10,256,4,4
       
        pred_score = F.conv2d(conv_scores, score_filters, groups=N).reshape(N, 10, self.score_displacement + 1,self.score_displacement + 1) #正负样本16，2*5，19,19
        #32,10,19,19
        conv_reg = conv_regression.reshape(1, -1, self.score_displacement + 4, self.score_displacement + 4)#1,16*256,22,22
        #32,256,22,22
        reg_filters = kernel_regression.reshape(-1, 256, 4, 4)#16*20,256,4,4
        #640,256, 4, 4
        pred_regression = self.regress_adjust(F.conv2d(conv_reg, reg_filters, groups=N).reshape(N, 20, self.score_displacement + 1,self.score_displacement + 1))#回归分支16，4*5，22,22
        #32, 20, 19, 19
